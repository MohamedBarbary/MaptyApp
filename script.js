...
const run2 = new Cycling([30, -12], 27, 95, 523);
...
class App {
  ...
  // this keyword
  // callback --> undifine
  // add event OR any handler  --> dom element
  // to edit this of them in class use bind mehtod
  _loadMap(position) {
  ...
  _newWorkout(e) {
    ...
    // if workout cycling , creating cyle object
    if (type === 'cycling') {
  ...
  _moveToPopup(e) {
    ...
    if (!workoutE) return;
    const workout = this.#workouts.find(w => w.id === workoutE.dataset.id);
